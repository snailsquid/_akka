...
Outbound WhatsApp messages are gated so that the throttle and idempotency invariants hold even across restarts and WAHA replays. The capability covers (a) one send queue per WAHA session, (b) duplicate-message suppression at the webhook boundary, and (c) the diagnostic signals that prove the invariants are holding.

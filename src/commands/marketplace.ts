...
export class MarketplaceHandler {
	...
	async showPage(
		...
		const items = pageCommands.map((cmd, i) => {
			const fullId = commandRegistry.buildFullId(
	...
	async handleResponse(
		...
		if (input === "n") {
			...
			return {
				completed: false,
				message: text,
				newState: { ...currentState, page: newPage },
			};
		}
		...
		if (currentState.awaitingCollision) {
			...
			if (input === "replace") {
				...
				userService.installCommand(
					userId,
					contactId,
					collision.newCommandId,
					collision.conflictSlug,
				);
			...
			return {
				...
				// No newState → router knows this is undealt, cancels flow, re-parses as command
			};
		}
		...
		if (!isNaN(num) && num > 0) {
			...
			if (!selected) {
				return {
					...
					// No newState → undealt, flow cancels and re-parses as command
				};
			}
			...
			if (userService.isInstalled(userId, contactId, selected.slug)) {
				const existingInstall = userService.getInstallation(
					userId,
					contactId,
					selected.slug,
				);
				const existingCmd = existingInstall
					? db
							...
							.where(eq(schema.commands.id, existingInstall.commandId))
							.get()
					: null;
				const existingDev = existingCmd
					? db
							...
							.where(eq(schema.developers.id, existingCmd.developerId))
							.get()
					: null;
		...
		// Unknown input — undealt, flow cancels and re-parses as command
		return {
			completed: false,
			message:
				"Reply with a number to install, 'n' for next page, or 'p' for previous.",
		};
	}

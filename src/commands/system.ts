...
async function handleHelp(ctx: SystemCommandContext): Promise<void> {
	const installations = userService.getUserInstallations(
		ctx.userId,
		ctx.contactId,
	);
...
async function handleRename(ctx: SystemCommandContext): Promise<void> {
	...
	const success = userService.renameInstallation(
		ctx.userId,
		ctx.contactId,
		oldSlug,
		newSlug,
...
async function handleLogin(ctx: SystemCommandContext): Promise<void> {
	...
	// Check if this WhatsApp JID is already linked to an existing developer
	const existingDev = db
		.select()
		.from(schema.developers)
		.where(eq(schema.developers.whatsappJid, ctx.userJid))
		.get();

	if (existingDev) {
		// Existing developer — link token to them, create session, mark used
		db.update(schema.registrationTokens)
			.set({ used: true, developerId: existingDev.id, jid: ctx.userJid })
		...
		await ctx.wahaClient.sendMessage(
			ctx.userJid,
			`✅ Welcome back, @${existingDev.username}!\n\nYou can now access the developer dashboard.`,
		);
		return;
	}

	// New developer — create with temp username, link JID, start signup flow
	const tempUsername = `dev_${token}`;
	const newDev = db
		.insert(schema.developers)
		.values({
			username: tempUsername,
			whatsappJid: ctx.userJid,
		})
		.returning()
		.get();

	// Mark token used and link to new developer
	db.update(schema.registrationTokens)
		.set({ used: true, developerId: newDev.id, jid: ctx.userJid })
		.where(eq(schema.registrationTokens.id, regToken.id))
		.run();

	// Start a developer_signup conversation flow
	const flowExpiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
	db.insert(schema.conversationFlows)
		.values({
			userId: ctx.userId,
			contactId: ctx.contactId,
			flowType: "developer_signup",
			state: "awaiting_username",
			data: JSON.stringify({ token, developerId: newDev.id }),
	...
	await ctx.wahaClient.sendMessage(
		ctx.userJid,
		"🎉 New number detected! Welcome to the Developer Dashboard.\n\nPlease reply with your desired username (letters, numbers, hyphens, underscores only):",
	);
}

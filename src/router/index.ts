...
export class Router {
	...
	constructor(
		wahaBaseUrl: string = process.env.WAHA_BASE_URL || "http://localhost:3001",
	) {
	...
	parseCommand(
		...
		for (const slug of sortedSlugs) {
			if (textAfterDot.toLowerCase().startsWith(slug.toLowerCase())) {
				...
				const args = rest
					? rest.split(/\s+/).filter((a: string) => a.length > 0)
					: [];
		...
		const potentialSlug =
			firstSpace === -1 ? textAfterDot : textAfterDot.slice(0, firstSpace);
	...
	startFlow(
		...
		const flows = db
			.select()
			.from(schema.conversationFlows)
			.where(eq(schema.conversationFlows.userId, userId))
			.all();
	...
	getActiveFlow(userId: number, contactId: number): ConversationFlow | null {
		...
		const flow = db
			...
			.where(
				and(
					eq(schema.conversationFlows.userId, userId),
					eq(schema.conversationFlows.contactId, contactId),
					gt(schema.conversationFlows.expiresAt, now),
				),
			)
			.get();
		if (!flow) return null;
	...
	endFlow(flowId: number): boolean {
		db.delete(schema.conversationFlows)
			.where(eq(schema.conversationFlows.id, flowId))
			.run();
		return true;
	}
	...
	updateFlow(
		...
		db.update(schema.conversationFlows)
			.set({ state, data: JSON.stringify(data), expiresAt })
			.where(eq(schema.conversationFlows.id, flowId))
			.run();
	}
	...
	async handleIncomingMessage(
		...
		if (!client) {
			console.error(`[Router] No client for contact ${contactId}`);
			return;
		}
		...
		if (activeFlow) {
			const flowResult = await this.handleFlowMessage(
				client,
				senderJid,
				messageId,
				body,
			...
			if (flowResult.consumed) return; // Flow consumed — done
			// Flow didn't consume (undealt) — fall through to command parsing
		}

		// Parse command
		command = this.parseCommand(body, userId, contactId);
		if (!command) return;
		...
		try {
			...
			const isInstalled = userService.isInstalled(
				userId,
				contactId,
				command.slug,
			);
			...
			if (isSystem) {
				...
				await handleSystemCommand(
					...
					async (uid, cid) => {
						...
						const initialState = await marketplaceHandler.buildInitialState(
							uid,
							cid,
						);
						this.startFlow(
				...
				await client.sendMessage(
					senderJid,
					"❌ Command not installed. Use .marketplace to browse.",
				);
			}

			await client.removeReaction(messageId, senderJid, "\u{1F441}");
		} catch (error) {
	...
	private async executeCommandCmd(
		...
		const installation = userService.getInstallationByUserSlug(
			userId,
			contactId,
			command.slug,
		);
		if (!installation) {
			await client.sendMessage(
				senderJid,
				"Command not found in your installations.",
			);
			return;
		}
		...
		const cmdRecord = db
			...
			.where(eq(schema.commands.id, installation.commandId))
			.get();
		if (!cmdRecord) {
		...
		// Get cached source
		let source = commandRegistry.getCommandSource(
			cmdRecord.developerId,
			cmdRecord.slug,
		);
		if (!source) {
			// Fetch and cache
			try {
				source = await commandRegistry.fetchCommandRepo(
					cmdRecord.repoUrl,
					cmdRecord.entryPoint,
				);
				commandRegistry.setCached(
					`${cmdRecord.developerId}/${cmdRecord.slug}`,
					source,
				);
			} catch {
				await client.sendMessage(
					senderJid,
					"❌ Failed to load command code. The repository may be unavailable.",
				);
				return;
			}
		}
		...
		const ctx = commandExecutor.createExecutionContext(
			...
			async (emoji) => {
				await client.sendReaction(messageId, senderJid, emoji);
			},
			async (_duration, _callback) => {
				// Schedule is wired to scheduler (stub for now)
				await client.sendMessage(
					senderJid,
					"⏰ Scheduler not yet wired. Coming soon!",
				);
			},
			user?.anonymizedId || "unknown",
			command.args,
			command.args.join(" ")
				? `.${command.slug} ${command.args.join(" ")}`
				: `.${command.slug}`,
			contactId,
		);
	 ...
	 * Returns a result indicating whether the message was consumed by the flow,
	 * and whether the flow should remain active or be cancelled.
	 */
	...
	): Promise<{ consumed: boolean; active: boolean }> {
		console.log(
			`[Router] Flow message for flow ${flow.id}, type: ${flow.flowType}, state: ${flow.state}`,
		);

		// Marketplace flow
		if (flow.flowType === "marketplace" || flow.flowType === "start") {
			let state = flow.data as unknown as {
				page: number;
				totalPages: number;
				commands: unknown[];
				awaitingCollision?: unknown;
			};
			if (!state.page) {
			...
			const result = await marketplaceHandler.handleResponse(
				...
				body,
				{
					...
					commands: state.commands as Array<{
						id: number;
						fullId: string;
						name: string;
						description: string;
					...
					awaitingCollision: state.awaitingCollision as
						| {
								newCommandId: number;
								newSlug: string;
								conflictSlug: string;
			...
			if (result.completed) {
				// Fully consumed (e.g. install succeeded) — send result, end flow
			...
			if (result.newState) {
				// Flow still active (navigation / collision resolution) — update state, keep flow alive
				this.updateFlow(
					flow.id,
					"browsing",
			...
			// Undealt input — cancel flow, re-process as command
			this.endFlow(flow.id);
			await client.sendMessage(senderJid, result.message);
